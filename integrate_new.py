...
def print_QOCC_Stats(Cons_val, Cost_original, Cost_Total, eta, iter, totalnumIter, res, inner_tol):
    print(
        "Cost_Constraint=%3e, Cost_original=%3e, eta=%3e, numIter=%d, totalnumIter=%d, inner_res=%.3g, inner_tol=%.3g" % \
    ...
    if n_contacts != 0:
        ...
        if params.compatibility is not None:
            ...
            if params.compatibility == "old_normals":
                ...
                QP_solver = solver(params.solver,
                                   const_N=None, project_from_old_normal=False, old_forces=params.old_forces,
                                   # f_star #params.old_forces,
                                   project_from_old_normal_oldTangents=True)

                x_guess = np.ones(3 * n_contacts)
                x_s = f_star - params.old_forces
                ## The increment of the tangent force satisfy the minimum norm
                ...
                print("unsupoorted compatibility criteion : '", params.compatibility,
                      "'. Choose from:\nold_normals\nIPM\nold_slips\ndecoupled_PD")
                pass
            ...
            for iter in range(params.compatibility_max_iter):
                f_NEW = QOCC_Augmented_Lagrangian(Cost_N=Cost_N,
                                                  ...
                                                  tau_eta=params.solver["AL_tau_eta"],
                                                  verbose=params.solver["AL_verbose"])

                params.f_star = f_star
                # no need to conitue
                if (
                        params.compatibility == "incremental" or params.compatibility == "old_normals" or params.compatibility == "IPM"):
                    break
