...
class solver:
    def __init__(self, solver_params, const_N=None, project_from_old_normal=False, old_forces=None,
                 project_from_old_normal_oldTangents=False):
    ...
    def solve(self, N, p, x0, friction, eps, max_iterations=None):
        ...
        if self.type == "Gauss_Seidel":
            x, iter, res = self.Gauss_Seidel(N, p, friction, eps, max_iterations, x0, self.params["GS_omega"],
                                             self.params["GS_lambda"])
        elif self.type == "Jacobi":
            x, iter, res = self.Jacobi(N, p, friction, eps, max_iterations, x0, self.params["Jacobi_omega"],
                                       self.params["Jacobi_lambda"])
