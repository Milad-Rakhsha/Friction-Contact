...
def main(out_dir):
    ...
    setup = {"nb": (HALF_ROW+1)*(NUM_PAR+1)+ HALF_ROW*NUM_PAR +3,
        ...
        "unique" : False,
        "Reg_T_Dir": False,
        "compatibility": "incremental", #"old_normals", #"old_normals", #"old_normals", #"old_slips", #None, "IPM", "decoupled_PD"
    ...
    model_params.add_box(np.array([sphere_radius*(NUM_PAR),0,-sphere_radius-box_hdims[2]]), rot, box_hdims, box_mass, id, fixed=True)
    ...
    while t <= model_params.time_end+model_params.dt:
        ...
        if  t >= t_settling:
            pushing = True
            f_push=FORCE_MG*gravity*sphere_mass*(1 if t>=t_pushing else (t-t_settling)/(t_pushing-t_settling))
